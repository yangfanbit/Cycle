#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Structural Analogy Rule Calibration v0.4 —— 在 R01 扩展 universe（79 cycle / 395 pairs）上校准 Rule Set v0.3。
...
OUT_JSON = f"{REP}/structural_analogy_rule_calibration_v0_4.json"
OUT_CSV = f"{REP}/structural_analogy_rule_calibration_pairs_v0_4.csv"

R3 = json.load(io.open(f"{REP}/structural_analogy_research_v0_4.json", encoding="utf-8"))
M3 = R3["matrix"]
...
SENS = {
    ...
    "note": ("本表为**规则变更的直接效应**；不得解释为性能变化，仅用于定位变化来源方向。"
             ...
             "**剩余 5 项 collision 处理（v0.3 → v0.4）不改变任何 Structural Status**。"),
}

res = {
    "artifact": "structural_analogy_rule_calibration",
    "artifact_version": "0.4",
    "generated_by": "research/scripts/build_structural_analogy_rule_calibration_v0_4.py",
    ...
    "universe": {
        ...
        "driver_data": {"v0_3": "historical_driver_evidence_ledger_v0_3.json（v0.3-r2 词表）",
                        "v0_4": "historical_driver_evidence_ledger_v0_4.json（v0.4：3 删词 + 2 上下文抑制）"},
    },
    "rule_change": {
        "id": "E.1",
    ...
    "E1_rule_effect": {
        "description": "同 driver 数据（Canonicalization **v0.4**），只换规则：v0.2 实现 → v0.3 实现（B 与 D 的对照）",
    ...
    "E2_canonicalization_effect": {
        "description": ("同规则（v0.2 实现），只换 driver 数据：Canonicalization **v0.3 → v0.4**"
    ...
    "combined_v0_2_baseline": {
        "description": "（v0.2 规则 + v0.3 driver）→（v0.3 规则 + v0.4 driver）",
