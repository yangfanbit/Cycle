"""Structural Analogy Explanation Artifact v0.4 —— Product-facing，基于 **Research v0.4**（79 Historical Objects · Rule Set v0.3 · Driver Canonicalization v0.4）。
...
OUT_JSON = f"{REP}/structural_analogy_explanations_v0_4.json"
...
# ★ v0.4：driver 逐条证据（供 dimension_evidence 保留 raw_driver / references）
...
for cp in R3["candidate_profiles"]:
    ...
    for m in sorted(rows, key=lambda x: x["historical_cycle"]):  # 稳定 identity 顺序
        ...
        # ★ v0.4 修正：三维数组必须**完整划分**四维（互不重叠且并集 = 4 维）
        ...
        # ★ v0.4 修正（契约 5.1）：`theme_relation` **不得**进入 why_not_similar ——
        ...
        dim_ev = {
            ...
            "mechanism_driver": [{
                ...
                # ★ v0.4：保留逐条 raw_driver（provenance 不丢失）
                ...
                "references": next((e["references"] for e in DRV_ROWS.get(cyc, [])
                                    if e["canonical_driver"] == k and e["bucket"] in ("start", "accelerator")), []),
                "note": ("raw → canonical（Historical Driver Canonicalization **v0.4**；"
                         "规则集 = Rule Set v0.3）"),
...
res = {
    ...
    "generated_by": "research/scripts/build_structural_analogy_explanation_v0_4.py",
    ...
    "contract": {
        ...
        "source_artifact": "research/research/reports/structural_analogy_research_v0_4.json",
    },
    ...
    "changelog_v0_3_to_v0_4": [
        {"id": "U1", "issue": "消费 Driver Canonicalization v0.2（旧）",
         "fix": "改为消费 **Driver Canonicalization v0.4**（含剩余 5 项 collision 处理）"},
