#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Historical Driver Canonicalization v0.4（deterministic，只读）。

**与 v0.3 的关系**：v0.4 = v0.3-r2 + **剩余 5 项 substring collision 的一次性处理**。
**首次引入 mapping algorithm 变更**（上下文抑制），故按版本纪律新建 v0.4 而非 v0.3-r3。
...
**与 v0.1 / v0.2 的关系**
  ...
  · v0.3 = 关键词表扩展；**v0.4 = 关键词删除 + 上下文抑制（见下）**。
...
产物：
  research/research/reports/historical_driver_canonicalization_v0_4.json
  research/research/reports/historical_driver_evidence_ledger_v0_4.json
...
OUT_CANON = f"{REP}/historical_driver_canonicalization_v0_4.json"
OUT_LEDGER = f"{REP}/historical_driver_evidence_ledger_v0_4.json"
...
ADDED_RULES = {
    ...
    "POLICY_DRIVEN": [
        ...
        # ★ v0.4-r1（P4）：删除「装备采购」—— 零增量覆盖（其唯一命中已被对偶词覆盖），见文件头 REVISIONS。
        ("军费", "军费 / 国防预算（政策—军费层）"),
        ("国防预算", "军费 / 国防预算（政策—军费层）"),
    ...
    "SUPPLY_CONTRACTION": [
        ...
        # ★ v0.4-r1（P2）：删除「整治提升」—— 零增量覆盖（其唯一命中已被对偶词覆盖），见文件头 REVISIONS。
    ...
    "VALUATION_RESET": [
        ...
        # ★ v0.4-r1（P5）：删除「黄金储备」—— 零增量覆盖（其唯一命中已被对偶词覆盖），见文件头 REVISIONS。
...
REVISIONS = [
    ...
    },
    {
        "revision": "v0.4-r1",
        "date": "2026-09-23",
        "change": ("剩余 5 项 substring collision 的**一次性处理**：3 项删词（B）+ 2 项最小上下文消歧（C）"),
        "scope": "仅这 5 项；未新增任何 canonical vocabulary；未改动映射状态语义",
        "decisions": [
            {"pair": "增长 / 负增长", "treatment": "C 最小上下文消歧",
             "rule": "`增长` 在**否定语境**（前置 1 字 ∈ {负,零,不,未,无}）下不计 `DEMAND_SURGE`",
             "why_not_B": ("删 `负增长` → 「新能源汽车7月起负增长」会得到 `DIRECT/DEMAND_SURGE`（**语义错误**）；"
                           "删 `增长` → 11 条真实正增长文本丢失需求信号 + 2 条负面变化"),
             "evidence": "`负增长` 的 10 条命中是 `增长` 21 条命中的**严格子集**；10 条中 9 条因「负增长」内含「增长」而**必然双命中**"},
            {"pair": "整治 / 整治提升", "treatment": "B 删词（删 `整治提升`）",
             "why": "零增量覆盖（唯一命中已被「整治」+「压减」覆盖）· 删除影响 = **0 条条目变化**（完全惰性）",
             "evidence": "`整治提升` 1 条命中 ⊂ `整治` 5 条命中"},
            {"pair": "出口 / 出口管制", "treatment": "C 最小上下文消歧",
             "rule": "`出口` 在**限制/管制语境**（前置 ≤8 字含 管制/限制/禁令/配额/禁止/制裁/收紧）下不计 `DEMAND_SURGE`",
             "why_not_B": ("删 `出口管制` → 「出口管制预期强化替代紧迫性」会得到 `DEMAND_SURGE`（**语义错误**，应为 POLICY）；"
                           "删 `出口` → 10 条变化、**6 条负面**、丢失 12 条真实出口需求命中"),
             "evidence": "`出口管制` 1 条命中 ⊂ `出口` 18 条命中；18 条中 5 条为**限制语境**（出口禁令 / 出口配额 / 禁止出口 / 出口限制 / 出口管制）"},
            {"pair": "采购 / 装备采购", "treatment": "B 删词（删 `装备采购`）",
             "why": ("零增量覆盖 · 删除后 2 条 `C-2020-MIL-EQUIP-ORDER` 文本由 `DERIVED/None` 变为 "
                     "`DIRECT/DEMAND_SURGE`，与**研究已确立的 driver**（该 Campaign 的 canonical driver = DEMAND_SURGE）一致"),
             "evidence": "`装备采购` 2 条命中 ⊂ `采购` 11 条命中；删除影响 = 0 个 driver 集合变化、0 负面"},
            {"pair": "储备 / 黄金储备", "treatment": "B 删词（删 `黄金储备`）",
             "why": "零增量覆盖 · 删除后「中国黄金储备连续第 11 个月上升」归 `POLICY_DRIVEN`（央行储备管理 = 政策行动），语义更准确",
             "evidence": "`黄金储备` 1 条命中 ⊂ `储备` 4 条命中；删除影响 = 0 个 driver 集合变化、0 负面"},
        ],
        "mapping_algorithm_change": {
            "introduced": True,
            "what": "新增 `CONTEXT_SUPPRESSION` + `keyword_hits()` —— **上下文抑制（只做减法）**",
            "discipline": [
                "**不新增任何 canonical 映射**（只抑制，不新增）→ 不引入新 vocabulary",
                "仅对 2 个关键词生效（`增长` / `出口`），规则为确定性字符串窗口判定",
                "precedence：先做上下文抑制 → 再按命中数判定 mapping_status（与 v0.3 顺序一致）",
            ],
        },
        "min_impact": {
            "changed_entries": 11,
            "canonical_driver_changes": 0,
            "primary_mechanism_changes": 0,
            "five_state": "DIRECT 55→58 · DERIVED 454→449 · AMBIGUOUS 2→2 · UNKNOWN 86→87 · NOT_AVAILABLE 98→99",
            "negative_changes": [
                "C-2018-HIEQ-ROBOT-DOWN accelerator#1：DERIVED → NOT_AVAILABLE"
                "（抑制后仅剩 CYCLE_REVERSAL，触发 v0.1 既有 `PRICE_ACTION` 排除（「调整」）→ 标签层面变化，**无 driver 损失**）",
                "RC-2022-RES-FERTILIZER accelerator#2：「中国禁止磷肥出口」原为 DERIVED/DEMAND_SURGE → UNKNOWN"
                "（**语义修正**：出口禁令 ≠ 需求放量；该 cycle driver 集合不变）",
            ],
        },
        "unchanged": ["canonical vocabulary（9 项）", "v0.1 / v0.2 / v0.3 产物", "mapping_status 语义", "Rule Set v0.3"],
    },
]
...
# ---------------------------------------------------------------- ★ v0.4 上下文抑制（mapping algorithm 变更）
# **只做减法**：当关键词的**该次出现**处于指定语境时，该次命中不计入。
# **不新增任何 canonical 映射** → 不引入新 vocabulary。
NEGATION_PREFIX = set("负零不未无")
RESTRICTION_MARKERS = ("管制", "限制", "禁令", "配额", "禁止", "制裁", "收紧")
CONTEXT_SUPPRESSION = {
    "增长": "NEGATION_PREFIX",       # P1：`增长`/`负增长` —— 否定语境（负/零/不/未/无 + 增长）不计 DEMAND_SURGE
    "出口": "RESTRICTION_WINDOW",    # P3：`出口`/`出口管制` —— 限制/管制语境不计 DEMAND_SURGE
}


def keyword_hits(text, kw):
    """关键词 `kw` 在 `text` 中**未被上下文抑制**的出现次数（v0.4 规则）。"""
    rule = CONTEXT_SUPPRESSION.get(kw)
    n = 0
    start = 0
    while True:
        ...
        if rule == "NEGATION_PREFIX":
            ...
            ok = not any(m in text[max(0, i - 8):i] for m in RESTRICTION_MARKERS)
        if ok:
            n += 1
        start = i + 1
    return n
    ...
    for bucket in ("start", "accelerator", "turning", "ending"):
        for idx, text in enumerate((o.get("drivers") or {}).get(bucket) or []):
            ...
            for canon, kws in RULES.items():
                for kw in kws:
                    # ★ v0.4：用 keyword_hits（含上下文抑制）替代朴素子串匹配；保留原大小写不敏感行为
                    if keyword_hits(t, kw) > 0 or keyword_hits(low, kw.lower()) > 0:
                        hits.append(canon)
                        break
...
# ---------------------------------------------------------------- v0.2 → v0.3 diff（可审计）
V2C = json.load(io.open(f"{REP}/historical_driver_canonicalization_v0_3.json", encoding="utf-8"))
V2L = json.load(io.open(f"{REP}/historical_driver_evidence_ledger_v0_3.json",
...
canon_doc = {
    "artifact": "historical_driver_canonicalization", "artifact_version": "0.4",
    "generated_by": "research/scripts/canonicalize_historical_drivers_v0_4.py",
    ...
    "summary": {
        ...
        "cycles_newly_covered_vs_v0_3": len(newly_covered),
    },
    "v0_3_to_v0_4_diff": {
...
ledger_doc = {
    "artifact": "historical_driver_evidence_ledger", "artifact_version": "0.4",
    "generated_by": "research/scripts/canonicalize_historical_drivers_v0_4.py",
