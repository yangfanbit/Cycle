"""Structural Analogy Research v0.3 —— 在 **R01 扩展后的 Historical Universe** 上执行**已冻结**的 Rule Set v0.2。
...
产物：
  ...
  research/research/reports/structural_analogy_research_candidates_v0_3.csv

用法：python research/scripts/build_structural_analogy_research_v0_3.py [--check]
"""
import collections
import csv
import io
...
OUT_JSON = f"{REP}/structural_analogy_research_v0_3.json"
OUT_CSV = f"{REP}/structural_analogy_research_candidates_v0_3.csv"

# ---------------------------------------------------------------- 冻结规则常量（Rule Set v0.2）
RULE_SET = "Structural Analogy Rule Set v0.2"
RULE_VER = "structural-analogy-ruleset-v0.2"
RULE_DOC = "research/research/methodology/structural_analogy_rule_set_v0_2.md"
...
CANON = json.load(io.open(f"{REP}/historical_driver_canonicalization_v0_2.json", encoding="utf-8"))
LEDGER = json.load(io.open(f"{REP}/historical_driver_evidence_ledger_v0_2.json",
...
V2P = f"{REP}/structural_analogy_research_v0_2.json"
...
if NONREG["available"]:
    ...
    _m2 = {(m["candidate_id"], m["historical_cycle"]): m for m in _v2["correspondence_matrix"]}
...
res = {
    "artifact": "structural_analogy_research", "artifact_version": "0.3",
    "generated_by": "research/scripts/build_structural_analogy_research_v0_3.py",
    ...
    "protocol": {
        ...
        "changed_vs_v0_2": ("**规则未变**；唯一变化 = Historical Universe 由 17 cycle 扩容至 79 cycle"
                            "（R01-01~R01-06 新增 52 campaigns + 27 research_candidates 全部纳入）"),
    },
    "inputs": {
        "current_candidates": "research/current/current_candidates.json",
        "export": "exports/timeline_export_v1.json",
        "driver_canonicalization": "research/research/reports/historical_driver_canonicalization_v0_2.json",
        "driver_ledger": "research/research/reports/historical_driver_evidence_ledger_v0_2.json",
    ...
    "universe": {
        ...
        "campaigns": sum(1 for h in HIST if h["kind"] == "campaign"),
        "research_candidates": sum(1 for h in HIST if h["kind"] == "research_candidate"),
        "current_candidates": len(CANDS),
        "pairs": len(V3),
        "macro_theme_roots_covered": sorted({h["macro_theme"] for h in HIST if h["macro_theme"]}),
        "v0_2_universe_cycles": 17,
