#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""validate_historical_research_intake.py —— Historical Universe Intake 包验证器（R00）。

校验 Research Agent 提交的 `Research Intake Package` 是否满足
`research/intake/HISTORICAL_UNIVERSE_INTAKE_PROTOCOL_v0_1.md`。

用法（在仓库根执行）:
    python research/scripts/validate_historical_research_intake.py <package>      # 目录或 .json
    python research/scripts/validate_historical_research_intake.py --check        # 基础设施自检
    python research/scripts/validate_historical_research_intake.py --list-checks  # 列出检查项
    python research/scripts/validate_historical_research_intake.py <pkg> --json   # 机器可读输出
...
INTAKE_DIR = os.path.join(ROOT, "research", "intake")
SCHEMA_PATH = os.path.join(INTAKE_DIR, "historical_research_intake.schema.json")
MANIFEST_PATH = os.path.join(
    INTAKE_DIR, "HISTORICAL_UNIVERSE_R01_TASK_MANIFEST_v0_1.json"
)
PACKAGES_DIR = os.path.join(INTAKE_DIR, "packages")
...
def _check_infrastructure_once() -> Report:
    ...
    if os.path.isdir(PACKAGES_DIR):
        ...
        rep.info("C02", "packages 目录尚不存在（R00 阶段正常）: %s" % PACKAGES_DIR)
