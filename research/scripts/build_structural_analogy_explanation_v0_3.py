"""Structural Analogy Explanation Artifact v0.3 —— Product-facing，基于 Research v0.3（R01 扩展 universe）。
...
OUT_JSON = f"{REP}/structural_analogy_explanations_v0_3.json"
...
for cp in R3["candidate_profiles"]:
    ...
    for m in sorted(rows, key=lambda x: x["historical_cycle"]):  # 稳定 identity 顺序
        ...
        unknown = [k for k, v in dims.items() if v["status"] in ("UNKNOWN", "NOT_AVAILABLE")]
        unsupported = [k for k, v in dims.items() if v["status"] == "MISMATCH"]
        ...
        if m["theme_relation"] == "CROSS_MACRO_THEME":
            why_not.append("跨 Macro Theme —— **这是背景 metadata，不是「不相似」的理由**")
        if not why_not:
        ...
        dim_ev = {
            ...
            "mechanism_driver": [{"canonical_driver": k,
                                  "mapping_status": "|".join(h["driver_provenance"].get(k, [])),
                                  "note": "raw → canonical（Historical Driver Canonicalization v0.2，"
                                          "规则与 v0.1 相同、覆盖全 universe）"}
...
res = {
    ...
    "generated_by": "research/scripts/build_structural_analogy_explanation_v0_3.py",
    "supersedes_note": ("v0.1（`structural_analogy_explanations_v0_1.json`）与 "
                        "v0.2（`structural_analogy_explanations_v0_2.json`）**逐字节保留**；"
    ...
    "contract": {
        ...
        "rule_set_version": RULE_SET_VER,
        "source_artifact": "research/research/reports/structural_analogy_research_v0_3.json",
    },
    ...
    "rule_set": RULE_SET,
    "rule_set_version": RULE_SET_VER,
    "changelog_v0_2_to_v0_3": [
        {"id": "U1", "issue": "历史 universe 仅 17 cycle（R01 前）",
         "fix": "扩容至 79 cycle（52 campaigns + 27 research_candidates），组合数 85 → 395"},
        {"id": "U2", "issue": "无 governance 元数据",
        ...
        {"id": "U3", "issue": "driver canonicalization 仅覆盖 17 cycle",
