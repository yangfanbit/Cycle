#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Structural Analogy Rule Calibration v0.3 —— 在 R01 扩展 universe（79 cycle / 395 pairs）上校准 Rule Set v0.3。
...
OUT_JSON = f"{REP}/structural_analogy_rule_calibration_v0_3.json"
OUT_CSV = f"{REP}/structural_analogy_rule_calibration_pairs_v0_3.csv"

R3 = json.load(io.open(f"{REP}/structural_analogy_research_v0_3.json", encoding="utf-8"))
M3 = R3["matrix"]
HP = {h["cycle_id"]: h for h in R3["historical_profiles"]}
CP = {c["candidate_id"]: c for c in R3["candidate_profiles"]}

LED2 = json.load(io.open(f"{REP}/historical_driver_evidence_ledger_v0_2.json",
                         encoding="utf-8"))["entries"]
LED3 = json.load(io.open(f"{REP}/historical_driver_evidence_ledger_v0_3.json",
                         encoding="utf-8"))["entries"]
C2 = json.load(io.open(f"{REP}/historical_driver_canonicalization_v0_2.json", encoding="utf-8"))
C3 = json.load(io.open(f"{REP}/historical_driver_canonicalization_v0_3.json", encoding="utf-8"))
...
STILL = C3["v0_2_to_v0_3_diff"]["still_without_canonical_driver"]
...
SENS = {
    ...
    "note": ("本表为**规则变更的直接效应**（同 driver 数据时见 E1_rule_effect）；"
             "不得解释为性能变化，仅用于定位变化来源方向。"),
}

res = {
    "artifact": "structural_analogy_rule_calibration",
    "artifact_version": "0.3",
    "generated_by": "research/scripts/build_structural_analogy_rule_calibration_v0_3.py",
    ...
    "universe": {
        ...
        "driver_data": {"v0_2": "historical_driver_evidence_ledger_v0_2.json（关键词表 = v0.1）",
                        "v0_3": "historical_driver_evidence_ledger_v0_3.json（关键词表扩展）"},
    },
    "rule_change": {
        "id": "E.1",
    ...
    "E1_rule_effect": {
        "description": ("同 driver 数据（Canonicalization **v0.3**），只换规则："
                        "v0.2 实现 → v0.3 实现（C 与 D 的对照）"),
    ...
    "E1b_published_baseline_rule_effect": {
        "description": ("同 driver 数据（Canonicalization **v0.2**，即 SA v0.3 已发布所用），"
                        "只换规则：v0.2 实现 → v0.3 实现（A 与 C 的对照）—— "
                        "用于直接说明**已发布 SA v0.3 中 50/89 的 MISMATCH-PARTIAL 由规则产生**"),
    ...
    "E2_canonicalization_effect": {
        "description": "同规则（v0.2 实现），只换 driver 数据：Canonicalization v0.2 → v0.3",
        "before_v0_2_drivers": counts(A),
        "after_v0_3_drivers": counts(B),
        "coverage_before": COV2,
        "coverage_after": COV3,
        "changed_pairs": len(E2),
        "by_transition": dict(collections.Counter("%s→%s" % (d["from"], d["to"]) for d in E2)),
    },
    "combined_v0_2_baseline": {
        "description": ("SA v0.3 **已发布结果**（v0.2 规则 + v0.2 driver）→ **新 v0.3 结果**"
                        "（v0.3 规则 + v0.3 driver）"),
        "published_sa_v0_3": counts(A),
        "new_v0_3": counts(D),
    ...
    "known_defects_recorded": [
        {
            ...
            "action": "**本轮只记录，不修改任何产物**（v0.1 / v0.3 逐字节保留）。建议在 SA v0.4 一并修正。",
        },
        {
            "id": "E.4",
            ...
            "action": "**本轮只记录**；多命中消歧属**映射逻辑**变更（非关键词变更），应留待 v0.4 评估。",
        },
    ],
