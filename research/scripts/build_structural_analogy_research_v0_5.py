"""Structural Analogy Research v0.5 —— 在 **79 Historical Objects** 上执行**已冻结**的 Rule Set v0.3。
...
产物：
  research/research/reports/structural_analogy_research_v0_5.json
  research/research/reports/structural_analogy_research_candidates_v0_5.csv

用法：python research/scripts/build_structural_analogy_research_v0_5.py [--check]
"""
import collections
import csv
import io
...
OUT_JSON = f"{REP}/structural_analogy_research_v0_5.json"
OUT_CSV = f"{REP}/structural_analogy_research_candidates_v0_5.csv"
...
HIST = []
# ★ v0.5 修正（E.3）：identity 必须**按来源数组**判定，不得用 `o.get("campaign_id")`。
    ...
    for cp in CANDS:
        ...
        for h in HIST:
            ...
            rows.append({
                ...
                "historical_kind": h["historical_object_kind"],   # ★ v0.5：按来源数组判定
...
res = {
    "artifact": "structural_analogy_research", "artifact_version": "0.5",
    "generated_by": "research/scripts/build_structural_analogy_research_v0_5.py",
    ...
    "protocol": {
        ...
        "changed_vs_v0_3": ("**规则未变**（仍为 Rule Set v0.3）；变化 = ① 消费 Driver Canonicalization v0.5"
