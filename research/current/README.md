...
## 11. 并行轮次交叉记录（2026-09-16 合并）

同一 `snapshot_date = 2026-09-15` 下存在**两个独立运行的研究轮次**：本轮（canonical，5 条候选）
与另一并行会话产出的轮次（提交 `6dac246`，4 条候选）。按交付优先级
**「规范统一 > 结构清晰 > 数据修正」**，canonical 采用本轮；并行轮次的完整内容保留在
Git 历史（`6dac246`）中，此处只登记要点，**不进入 canonical 数据集**。

### 11.1 并行轮次候选（未采纳）

| candidate_id | display_name | 声明状态 / 关注度 | 证据数 |
|---|---|---|---|
| `CC-2026-AI-EINFO` | AI 驱动电子信息制造升级与高速互联基础设施 | RESEARCHING / EXPANSION | 4 |
| `CC-2026-AI-ENERGY` | AI+能源与算电协同 | CANDIDATE / THEME_FORMING | 5 |
| `CC-2026-BCI-MEDICAL` | 脑机接口医疗器械产业化与标准化 | CANDIDATE / THEME_FORMING | 6 |
| `CC-2026-HUMANOID` | 人形机器人从量产扩张向规模化应用与标准化迁移 | WATCH / EXPANSION | 5 |

### 11.2 两轮的重叠与差异

下表是**语义对应**，不是同一研究对象 —— 两轮的粒度与边界不同：

| 并行轮次 | 本轮 | 说明 |
|---|---|---|
| `CC-2026-BCI-MEDICAL` | `CC-2026-BCI-MEDTECH` | 同一方向；本轮收敛到「标准—分类—临床—商业化」链路 |
| `CC-2026-AI-ENERGY` | `CC-2026-COMPUTE-POWER` | 同一方向；本轮收敛到「算力网 ↔ 新型电力系统」协同 |
| `CC-2026-HUMANOID` | `CC-2026-EMBODIED-AI` | 同一方向；本轮覆盖标准 / 实训 / 供应链，而非仅本体量产 |
| `CC-2026-AI-EINFO` | `CC-2026-OPTICAL-LINK`（部分） | 并行轮次把高速光互联并入「电子信息制造升级」大主题 |
| —（未覆盖） | `CC-2026-OFFSHORE-WIND` | 并行轮次未涉及海上风电 |

### 11.3 未采纳的理由（方法论层面，不是数据对错）

1. **相位证据矩阵的写入方式不同。** 并行轮次把 8 个维度**全部**写进候选声明的
   `phase_evidence`；本协议规定其中 5 维（policy / industry / market / capital / company）
   **必须由证据台账派生**、研究侧不得手工声明（§3.3）。允许手工声明会切断
   「声明阶段」与「证据是否支持」之间的可审计对照。
2. **`macro_theme` 使用新建名称。** 并行轮次引入 4 个历史数据中不存在的 Macro Theme
   （AI与电子信息制造 / AI与能源融合 / 脑机接口医疗器械 / 人形机器人与具身智能），
   后两者在粒度上更接近 Sub-theme；本轮复用既有 Macro Theme（医药健康 / 电力设备 /
   高端装备 / 信息通信），以保持与历史 Theme Cycle 的可对齐性（仍受 §9.1 的 Pattern 匹配问题影响）。
3. **候选粒度。** 并行轮次把「AI 总主题」下的多个环节合并为一个候选（`CC-2026-AI-EINFO`）；
   本轮选择按**产业环节**切开（高速光互联单独成对象），更贴合「可独立追踪里程碑」的要求。
4. **可复现性字段缺失。** 并行轮次未记录 `research_round` / `research_method` / `source_policy` /
   `known_limitations`，无法与下一轮同口径对照（§10.4）。

> 两轮 `snapshot_date` 相同、`AFTER_SNAPSHOT` 均为 0，且都通过 `validate_current_research.py`
> 六组校验 —— **分歧不是数据合规问题，而是粒度与矩阵写入协议问题**。
> 该分歧本身是 §10 的有用素材：下一轮可回看「按大主题切 / 按产业环节切」哪种更能被后续证据推进。
